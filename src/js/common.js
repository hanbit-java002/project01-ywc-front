define([
	...
	function getGalleryCookie() {
		...
		return galleryCookieList;
	}
	/* 쿠키 갤러리 세팅*/
	function setGalleryBannerImgs() {
		var items =getGalleryCookie();
		$(".recent-lists").remove();
		var htmlImgs="";
		for (var count=0; count<items.length; count++) {
			var item=items[count];
			console.log(item.galleryImg);
			htmlImgs+="<li name="+item.galleryId+" class=\"recent-lists\">";
			htmlImgs+="	<img src='"+global.root+"/img/"+item.galleryImg+"'>";
			htmlImgs+="	</li>";
			}
		$(".right-bar-contents>ul").append(htmlImgs);
	}

	setGalleryBannerImgs();
	logedIncheck();
	mainLogoEnter();
	mainLogoLeave();
	detailsEnter();
	...
	headerMenuFixed();
	return {
		getGalleryCookie: getGalleryCookie,
	};
});
